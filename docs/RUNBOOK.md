...
### 6.6 `git pull` сообщает о расходящейся истории (`divergent branches`)

Симптом:

- при `git pull` на сервере git просит явно выбрать стратегию pull
  (`rebase` или `merge`);
- обычно это означает, что `origin/main` уже обновился, а локальная
  `main` тоже содержит собственные коммиты.

Без изменения глобальных git-настроек (рекомендуемый безопасный вариант):
...
Альтернатива для более линейной истории:

- `git pull --rebase`

Как выбрать стратегию:

- `--no-rebase` (merge) — проще и безопаснее для ops-процедур на VPS;
- `--rebase` — чище история коммитов, но требует аккуратного
  разрешения конфликтов и понимания переписывания локальной истории.
