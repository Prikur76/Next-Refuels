...
2. Заполните минимум:
   ...
   - `FRONTEND_BASE_URL` (в шаблоне уже `http://localhost:5173`; при другом
     хосте порта фронта поправьте и при необходимости `CSRF_TRUSTED_ORIGINS`
     в `/.env.example`)
...
### 1.2) Фронт Next.js без Docker

Если поднимаете только `frontend/` (не весь compose):

- `Copy-Item frontend/.env.example frontend/.env.local` (или `cp` на Unix);
- настройте `NEXT_PUBLIC_API_URL` / `NEXT_PUBLIC_DJANGO_ADMIN_URL` по
  комментариям в `frontend/.env.example`;
- для запросов с фронта на `http://localhost:8000` включите CORS в `.env.dev`
  (`CORS_ALLOWED_ORIGINS`, см. `/.env.example`).

В Docker-стеке переменные `NEXT_*` для фронта заданы в
...
- `docs/ARCHITECTURE.md` — архитектура и **логика раздела «Аналитика»**
  (срезы дашборда, топливозаправщики, поля `GET /api/v1/analytics/stats`).
- `docs/RUNBOOK.md` — деплой, SSL, импорт CSV, окружение и CI/CD.
- `docs/SECURITY.md` — заметки по безопасности.
- `docs/TESTPLAN.md` — план проверок.
